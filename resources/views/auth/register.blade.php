    ...
    <div class="card border rounded p-4 mx-auto" style="max-width:520px; box-shadow:none">
        ...
        <form method="POST" action="{{ route('register') }}">
            ...
            <div class="mb-3">
                ...
                <input id="name" class="form-control rounded @error('name') is-invalid @enderror" type="text" name="name" value="{{ old('name') }}" required autofocus autocomplete="name">
                @error('name')
            ...
            <div class="mb-3">
                ...
                <input id="email" class="form-control rounded @error('email') is-invalid @enderror" type="email" name="email" value="{{ old('email') }}" required autocomplete="username">
                @error('email')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
            ...
            <div class="d-flex justify-content-between align-items-center gap-3 flex-wrap">
                <a class="small" style="color:#6600AA" href="{{ route('login') }}">Already registered?</a>
                <button type="submit" class="btn btn-primary-custom rounded">Register</button>
            </div>
        </form>
    </div>
@endsection
